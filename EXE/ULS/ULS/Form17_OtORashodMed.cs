...
namespace ULS
{
    public partial class Form17_OtORashodMed : Form
    {

        Object Range;
        Excel._Worksheet workSheet;
        Form1 frm1;
        public Form17_OtORashodMed(Form1 f)
        {
            InitializeComponent();
            frm1 = f;
        }
        ...
        private void Form17_OtORashodMed_Load(object sender, EventArgs e)
        {
        ...
        private void ExportToExcel()
        {
            ...
            if (count != 0)
            {
                ...
                workSheet.Cells[4, 8] = "Ед. измерения";
                workSheet.Cells[4, 9] = "Колич.";
                workSheet.Cells[4, 10] = "Цена за ед.";
                workSheet.Cells[4, 11] = "Общая сумма";
                SetColumnWidth("A1", 3);
                ...
                for (int j = 0; j < frm1.dataGridView1.Rows.Count - 1; j++)
                {
                    ...
                    if (col > 0)
                    {
                        ...
                        workSheet.Cells[rowExcel, "H"] = frm1.dataGridView1.Rows[j].Cells[6].Value;
                        workSheet.Cells[rowExcel, "I"] = col;
                        workSheet.Cells[rowExcel, "J"] = frm1.dataGridView1.Rows[j].Cells[8].Value;
                        workSheet.Cells[rowExcel, "K"] = col * (int.Parse(frm1.dataGridView1.Rows[j].Cells[8].Value.ToString()));
                        ++rowExcel;
                        obcol = obcol + col;
                ...
                SetBorderStyle("H" + (ocount + 5).ToString() + ":K" + (ocount + 5).ToString(), 1);
                workSheet.Cells[ocount + 5, 8] = "ИТОГО:";
                workSheet.Cells[ocount + 5, 8].EntireRow.Font.Bold = true;
                workSheet.Cells[ocount + 5, 9] = obcol.ToString();
                workSheet.Cells[ocount + 5, 11] = obsum.ToString();
                workSheet.Cells[ocount + 5, 9].EntireRow.Font.Bold = true;
                workSheet.Cells[ocount + 5, 11].EntireRow.Font.Bold = true;
            }
            else
